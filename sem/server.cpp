...
#define NUMMSG 200                /* Server only doing two "receives"  */
                                /*   on shm segment                  */
    ...
    for (i=0; i < NUMMSG; i++)
      {
        ...
        operations[0].sem_op  = -1; // Decrement the semval by 1
        operations[0].sem_flg = IPC_NOWAIT; // Do not allow to wait

        rc = semop( semid, operations, 1 );
        if (rc == -1)
          {
            printf("main: 2nd semop() failed\n");
            return -1;
        }
